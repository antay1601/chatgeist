...
logger.info("✅ Безопасный бот с Docker sandbox для Claude CLI")
...
def ask_claude_with_context_secure(question: str, history: list[dict], db_path: str = "telegram_messages.db") -> str:
    ...
    if history:
        ...
        full_prompt = f"""Ты находишься в Linux окружении. Для получения данных используй команду:
sqlite3 telegram_messages.db 'SQL QUERY'
...
Проанализируй данные в БД с учётом контекста предыдущих вопросов и ответов, и предоставь развернутый ответ."""
    else:
...
Проанализируй данные в БД и предоставь развернутый ответ."""
    ...
    try:
        ...
        # Добавляем stderr к ошибке для отладки
        error_details = f"Exit code: {e.returncode}\nStdout: {e.stdout}\nStderr: {e.stderr}"
        logger.error(f"Claude CLI error: {error_details}")
        raise RuntimeError(f"Claude CLI завершился с ошибкой:\n{e.stderr}") from e
...
async def handle_query(message: Message):
    ...
    try:
        ...
        error_str = str(e)
        if "не запущен" in error_str or "контейнер" in error_str.lower():
            ...
            # Ошибка Claude CLI
            logger.error(f"Ошибка Claude CLI: {e}")
            await status_msg.edit_text(
                f"❌ Ошибка при выполнении Claude CLI:\n\n`{error_str}`",
                parse_mode="Markdown"
            )
