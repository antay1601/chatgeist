...
Использует Google Gemini API для обработки запросов.
"""
...
logger.info("✅ Бот с Gemini API готов к работе")
...
def ask_gemini_with_context(question: str, history: list[dict], db_path: str = "telegram_messages.db") -> str:
    """
    Отправляет вопрос в Gemini API с контекстом базы данных.
...
ВАЖНО:
1. Если нужно получить данные из БД - сформируй SQL запрос и верни его в формате: SQL_QUERY: <твой запрос>
2. Используй ПРОСТОЕ текстовое форматирование (без Markdown)
3. Для заголовков используй === или ---
4. Для списков используй цифры или дефисы
...
Проанализируй вопрос и предоставь развернутый ответ."""
    ...
    logger.info(f"Отправка запроса в Gemini API (история: {len(history)} сообщений)")

    try:
        # Используем Gemini 2.5 Pro Preview
        ...
        if "SQL_QUERY:" in answer:
            ...
            # Отправляем результат обратно в Gemini для анализа
...
Для списков используй цифры или дефисы."""
        ...
        raise RuntimeError(f"Ошибка при обращении к Gemini API: {str(e)}")
...
🤖 Работаю на базе Google Gemini API
    """
...
async def handle_query(message: Message):
    ...
    try:
        ...
        import asyncio
        report = await asyncio.to_thread(ask_gemini_with_context, user_query, history, DB_PATH)
...
async def main():
    ...
    """
    logger.info("Запуск бота с Gemini API...")
